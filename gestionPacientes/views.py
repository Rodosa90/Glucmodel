...
from gestionPacientes.models import Usuarios, Paciente, Tratamiento, Pesos, Calorias, Ritmo_cardiaco, Pasos, Suenio, \
    Siesta, \
    Siesta_resumen, Suenio_resumen, Glucosa_freestyle, Evento_carbohidratos, Evento_insulina_lenta, \
    Evento_insulina_rapida, Medico, Centro_medico, Centro_investigacion, Investigador, Dietas
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            usuario = Usuarios(
                ...
                iddoctor = paciente.doctor_id_id ,
                fechaconsen =con_date ,
                hipergluc =request.POST['hiper'] ,
                hipogluc =request.POST['hipo']
           ...
           # Deshabilitado envio correo 29-05-2020
            ...
            return render(request, 'register.html',
                          {'obj': obj,'obj': obj, 'tipo_diabetes': tipo_diabetes, 'msg': "Usuario registrado con éxito"})

        else:
            return render(request,'register.html',{'obj': obj,'medicos': medicos, 'tipo_diabetes' : tipo_diabetes})
    except Exception as e:
        return render(request, 'register.html', {'obj': obj,'medicos': medicos, 'tipo_diabetes': tipo_diabetes,'msg': "Ha ocurrido un error"})
"""
#registro medico
        ...
        try:
           ...
           # investigadores = Investigador.objects.all()
            ...
            if request.method == 'POST':
                ...
                acronimo = str(Centro_medico.objects.filter(name=centroI)[0].acronym) + '{0:04d}'.format(len(Investigador.objects.all()) + 1) + 'I'

                # print(acronimo)
                for c in Centro_investigacion.objects.all():
                    if (centroI == c.name):
                        centro = c  # se asigan un centro medico
...
def registerP(request):
    try:
        ...
        if request.method == 'POST':
            ...
            acronimo = str(Centro_medico.objects.filter(name=centroM)[0].acronym) + '{0:04d}'.format(len(Paciente.objects.all()) + 1) + 'P'
            ...
            if Paciente.objects.filter(username = email).count():
                return render(request,"registerP.html",{'centros': centros,'msg': "El paciente " + email + " ya existe"})
            ...
            return render(request, 'registerP.html',
                          { 'centros': centros, 'msg': "Paciente registrado con éxito" , "user_ID": "Tu userID es:" + acronimo})

        else:
            return render(request,'registerP.html',{'centros': centros})
    except Exception as e:
        print(e)
        return render(request, 'registerP.html', { 'centros': centros,'msg': "Ha ocurrido un error"})


# registro Centro Médico
def registerCM(request):
        try:
            centros = Centro_medico.objects.all()
    ...
    try:
        ...
        for file in files:
            ...
            try:
                ...
                csv_file.seek(0)
                data_set = csv_file.read().decode('mbcs')  # lee los datos
                with codecs.open(name, "w", encoding="UTF-8") as targetFile:
                    targetFile.write(data_set)

                csv_file = codecs.open(name, encoding="UTF-8")
            csv_file.seek(0)
           # if extension[1] != 'csv':
...
def roche(request, csv_file,usuario):
    try:
        ...
        tipo=type( roche['Time'][0])
        timer=datetime.strptime(time[0], '%d/%m/%Y %H:%M')
        timer=timer.dt.round('5min') #falla
        """
        # pruebas--------------------------
       # roche['Time'] =datetime.strptime(roche['Time']) .round('5min') #falla
        # se ha tenido que especificar el formato de la fecha , ya provocaba incongruencias y eliminar lineas sin hora
        roche['Time'] = pd.to_datetime(roche['Time'],format= '%d/%m/%Y %H:%M' ,errors='coerce')

        roche['Time'] = roche['Time'].dt.round('5min')
        #nuevo --------
        roche.sort_values(by=['Time'])
        fecha_min =   roche['Time'][len(roche)-1]
        fecha_max =   roche['Time'][0]
        #----------------
        ...
        """
        #esto es de ser un genio, se trata una excepcion pero en el return de abajo se 
        retornan valores nulos, generando otra excepción y pisando el mensaje de este
        """

        msg = "Error en el tratamiento de datos de la bomba de Roche"+type(inst)+"  "+inst.args+"   "+inst
    ...
    try:
        ...
        try:
            ...
            try:
                #caso ; gramos
                csv_file.seek(0)
                gluc_data = pd.read_csv(csv_file_copy, skiprows=1,sep=';',
                                        usecols=['Hora', 'Glucosa leída (mg/dL)', 'Histórico glucosa (mg/dL)',
                                                 'Insulina de acción rápida sin valor numérico',
                ...
                try:
                    ...
                    gluc_data = pd.read_csv(csv_file_copy, skiprows=1, sep='\t',
                                            usecols=['Hora', 'Glucosa leída (mg/dL)', 'Histórico glucosa (mg/dL)',
                                                     ...
                                                     'Alimentos sin valor numérico',
                                                     'Carbohidratos (gramos)',
                                                     'Insulina de acción lenta sin valor numérico',
                                                     'Insulina de acción lenta (unidades)',
                                                     'Glucosa de la tira (mg/dL)',
                                                     'Cetonas (mmol/L)'])
                    ...
                    try:
                        ...
                        try:
                            ...
                            csv_file.seek(0)
                            gluc_data = pd.read_csv(csv_file_copy, skiprows=1, sep=',',
                                                    usecols=['Hora', 'Glucosa leída (mg/dL)',
                            ...
                            try:
                                ...
                                try:
                                    csv_file.seek(0)
                                    gluc_data = pd.read_csv(csv_file, skiprows=2, sep=',')
                                    if len(list(gluc_data)) == 19:
                                        gluc_data.columns = ["Dispositivo", "ID", "Hora", "Tipo de registro",
                                                                "Histórico glucosa (mg/dL)", "Glucosa leída (mg/dL)",
                                                                "Insulina de acción rápida sin valor numérico",
                                                                "Insulina de acción rápida (unidades)",
                                                                "Alimentos sin valor numérico", "Carbohidratos (gramos)",
                                                                "Carbohidratos (raciones)",
                                                                "Insulina de acción lenta sin valor numérico",
                                                                "Insulina de acción lenta (unidades)", "Notas",
                                        ...
                                        msg = "Archivo incompatible - se necesita separación por ; o tab, o culumnas incompatiples"
        ...
        # 5 minutes sampling
        gluc_data = gluc_data.resample('15min').mean()
        #prueba----
        gluc_data.to_csv('prueba.csv')
                ...
                if col == "Insulina de acción rápida (unidades)":
                    ...
                    for column in csv.reader(io_string, delimiter=',', quotechar="|"):  # inserta datos en la bd
                        ...
                        _, created = Dietas.objects.update_or_create(
                            date=column[0],
                            iduser=usuario,
                            defaults={"raciones": column[1], }
                        )
                elif col == "Carbohidratos (gramos)":
                    for column in csv.reader(io_string, delimiter=',', quotechar="|"):  # inserta datos en la bd
                        carbo=column[1]/10
                        _, created = Carb_input.objects.update_or_create(
                            time=column[0],
